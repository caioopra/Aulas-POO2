...
class ControladorSistemaEmpresas:
    ...
    def inclui_empresa(self, empresa: Empresa):
        # tenta buscar a empresa na lista das empresas
        try:
            self.__empresa_dao.get(empresa.cnpj)
        # caso nao encontre, ela e adicionada
        except KeyError:
            self.__empresa_dao.add(empresa)
        # caso busque e encontre (no try), iria duplicar e gera excecao
        else:
    ...
    def busca_empresa_pelo_cnpj(self, cnpj: int) -> Empresa:
        ...
        # caso nao exista a empresa, retorna objeto None
        except KeyError:
            return None
